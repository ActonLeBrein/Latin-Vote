...
$(document).ready(function(){   	
	...
	$("#btn_1").on("click", function(){				
		...
		mostrarinfo($("#selected").data("anio"),$("#tipo").val());
...
function colorearEdos(fip){
	$("#timeline div").removeClass("waca");
	for (i=1960;i<=2020;i++){		
			if	(jQuery.inArray(fip, registros[i]) != -1){	
				var pos = (i % 1960)*2;							
				$("#timeline div").eq(pos).addClass("waca");						
			}
...
function mostrar(){	
	$("#timeline").empty();
    var $line, $year, by_n, delta_position, diff, final_year, i, initial_position, initial_year, k, label, label_index, labels, mark_year, new_width, position, select_year, year, _i;
    window.previous_year = 0;
    initial_year = 1960;
    final_year = 2020;
    by_n = 1;
    k = by_n * 2;
    mark_year = 1;
    select_year = 1;
    diff = (final_year - initial_year + k) / by_n;
    delta_position = 12;
    new_width = delta_position * diff;
    $("#timeline").parent().attr("style", "width:" + new_width + "px;");
    initial_position = Math.floor($("#timeline").position().left);
    i = 1;
	var n ="NE";
	if (window.ver!="esp")
		n="NA";
    ...
    label_index = 0;
    for (year = _i = initial_year; initial_year <= final_year ? _i <= final_year : _i >= final_year; year = _i += by_n) {
      position = initial_position + (i * delta_position) - i;
      $line = $(document.createElement("div"));
      $line.addClass("line");
      $line.attr("style", "left: " + position + "px;");
      $line.data("left", position);
      $year = $(document.createElement("div"));
      $year.addClass("year");
      if ((i - 1) % mark_year === 0) {
        $year.addClass("year-mark");
      } else {
        $year.addClass("year-no-mark");
      }
      if ((i - 1) % select_year === 0) {
        label = labels[label_index];
        label_index++;
        $year.attr("data-year", label);
        $line.addClass("year-selector");
        $year.addClass("year-mark-active");
        $year.removeClass("year-mark");
		if (jQuery.inArray(label, [1960,1970,1980,1990,2000,2010,2020]) != -1){
			$year.html("<span>"+label+"</span>");
		}
    ...
    $(".line.year-selector").hover(function(e) {
      year = $(this).children(".year").data("year");
      $(".line.year-selector .year").removeClass("selected");
      $(this).children(".year").addClass("selected");
      $("#selected").data("anio",year);
      $("#selected").css({
        left: $(this).data("left") - $("#selected").width() / 2 + 5
      });
      if (window.previous_year !== year) {    	
		mostrarinfo(year,$("#tipo").val());
	  }
    });
    return $(".line.year-selector:first").mouseover();
}
